...
@Component({
  selector: 'jhi-dd-message-mp-ai',
  templateUrl: './dd-message-mp-ai.component.html'
})
export class DdMessageMpAiComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.ddMessageService
      ...
      .subscribe((res: HttpResponse<IDdMessageMpAi[]>) => this.paginateDdMessages(res.body, res.headers));
  }
  ...
  transition() {
    this.router.navigate(['/dd-message-mp-ai'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IDdMessageMpAi) {
    return item.id;
  }
  ...
  protected paginateDdMessages(data: IDdMessageMpAi[], headers: HttpHeaders) {
