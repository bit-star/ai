...
@Component({
  selector: 'jhi-process-msg-task-mp-ai',
  templateUrl: './process-msg-task-mp-ai.component.html'
})
export class ProcessMsgTaskMpAiComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.processMsgTaskService
      ...
      .subscribe((res: HttpResponse<IProcessMsgTaskMpAi[]>) => this.paginateProcessMsgTasks(res.body, res.headers));
  }
  ...
  transition() {
    this.router.navigate(['/process-msg-task-mp-ai'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IProcessMsgTaskMpAi) {
    return item.id;
  }
  ...
  protected paginateProcessMsgTasks(data: IProcessMsgTaskMpAi[], headers: HttpHeaders) {
